...
shinyServer(function(input, output, session) {
  ...
  output$plot_navig_buttons <- renderUI({
    if (simul$finished == FALSE)
      ...
      tagList(
        ...
        sliderTextInput(
          "slider_iter",
          label = NULL, choices = isolate(round((0:input$total_q)/input$total_q, 2)),
          selected = 1, grid = TRUE,
          animate = list(interval = 1800, loop = FALSE, playButton = NULL, pauseButton = NULL),
          width = "100%"
        )
      )
    }
  ...
  output$plot_scenario1 <- renderPlot({
    ...
    plot_incidence <-
      ...
      labs(title = paste0("Incidence — Quality = ", round(selected_q / isolate(input$total_q), 2)),
    ...
    plot_global <-
      ggplot(data = simul$results_s1 %>% filter(qq <= selected_q / isolate(input$total_q)), aes(x = qq)) +
      ...
      labs(title = "Impact of Medicine Quality", x = "Medicine quality level (proxy for API)",
